...
+
image:express-c-series-aff220-deploy_image41.png["VMware ESXi ホスト ページのスクリーンショット。「Get vCenter Server」ボタンがハイライト表示されています"]
...
+
image:express-c-series-aff220-deploy_image42.png["Embedded Platform Services Controller が選択された状態での vCenter Server Appliance の展開タイプ選択のスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image43.png["vCenter Server アプライアンスのデプロイメントターゲットページのスクリーンショット（ESXi ホスト、ポート、root ユーザー、パスワードの各フィールドを含む）"]

. VCSA を VM 名として「 VCSA 」に入力し、 VCSA に使用するルート・パスワードを設定します。
+
image:express-c-series-aff220-deploy_image44.png["VM 名と root パスワードのフィールドを含むアプライアンス VM セットアップページのスクリーンショット"]

. 環境に最も適した導入サイズを選択してください。次へをクリックします。
+
image:express-c-series-aff220-deploy_image45.png["vCenter Server の展開サイズページのスクリーンショット（Tiny と Default ストレージが選択された状態）"]

. infra_datastore_1 データストアを選択します。次へをクリックします。
+
image:express-c-series-aff220-deploy_image46.png["vCenter Server インストーラーの「データストアの選択」ページで infra_datastore_1 が選択されているスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image47.png["vCenter Server Appliance インストーラーの静的 IPv4 フィールドが入力されたネットワーク設定の構成画面のスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image48.png["vCenter Server Appliance のステージ 2 の概要ページのスクリーンショット（「次へ」ボタン付き）"]
