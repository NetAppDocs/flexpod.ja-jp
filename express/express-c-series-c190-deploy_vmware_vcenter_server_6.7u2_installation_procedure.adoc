...
+
image:express-c-series-c190-deploy_image34.png["vCenter Server Appliance インストーラーのエンドユーザーライセンス契約ページのスクリーンショット（同意を選択済み）"]

. 展開タイプとして、 Embedded Platform Services Controller を選択します。
+
image:express-c-series-c190-deploy_image35.png["vCenter Server Appliance Installer の展開タイプページで「Embedded Platform Services Controller」が選択されているスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image36.png["ESXiホスト認証情報を含むアプライアンス展開ターゲットフォームが表示されたvCenter Server Appliance Installer のスクリーンショット"]

. vCSA に VM 名および root パスワードとして入力し、 vCSA に使用するアプライアンス VM を設定します。
+
image:express-c-series-c190-deploy_image37.png["vCenter Server Appliance インストーラーの「アプライアンス VM の設定」ページのスクリーンショット（VM 名と root パスワードのフィールドを表示）"]

. 環境に最も適した導入サイズを選択してください。次へをクリックします。
+
image:express-c-series-c190-deploy_image38.png["vCenter Server Appliance インストーラーの「展開サイズの選択」ページで「Tiny」が選択されているスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image39.png["vCenter Server Appliance インストーラーの「ネットワーク設定の構成」ページで MGMT-Network が選択されているスクリーンショット"]

. アプライアンスの導入を開始する前に、第 1 段階の設定を確認してください。
+
image:express-c-series-c190-deploy_image40.png["vCenter Server Appliance インストーラーの「ステージ1の設定確認（完了の準備ができました）」ページのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image41.png["vCenter Server Appliance インストーラーのステージ 2 の「はじめに」ページのスクリーンショット（「次へ」ボタン付き）"]
...
+
image:express-c-series-c190-deploy_image42.png["新しい SSO ドメインが選択された状態の vCenter Server Appliance インストーラー ステージ 2 の SSO 構成のスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image43.png["vCenter Server Appliance インストーラーの「CEIP の設定」ページで CEIP オプションが選択された状態のスクリーンショット"]

. 設定の概要を確認します。[ 完了 ] をクリックするか、 [ 戻る ] ボタンを使用して設定を編集します。
. インストールの開始後に、インストールを一時停止または終了できないことを示すメッセージが表示されます。[OK] をクリックして続行します。
+
image:express-c-series-c190-deploy_image44.png["インストールを一時停止または停止できないことを警告する vCenter Server Appliance インストーラーのスクリーンショット（「キャンセル」ボタンと「OK」ボタンが表示されています）"]
