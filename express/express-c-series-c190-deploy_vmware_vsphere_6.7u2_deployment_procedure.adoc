...
+
image:express-c-series-c190-deploy_image17.png["Cisco CIMC サーバーの BIOS ページのスクリーンショット（上部メニューに「KVM を起動」リンクあり）"]
...
=== ESXi 6.7U2 Cisco カスタム イメージのダウンロード
...
+
image:express-c-series-c190-deploy_image18.png["容量15.00 GiBのNETAPP LUN C-Modeストレージデバイスを選択しているESXiインストーラーのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image19.png["管理ネットワーク用に vmnic2 と vmnic4 が選択された ESXi ネットワークアダプタ画面のスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image20.png["vSwitch0 と iScsiBootvSwitch を示す仮想スイッチタブが表示された ESXi ネットワークページのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image21.png["vmnic3がアクティブでvmnic5が未使用の場合のiSCSIBootPG-Aのチーミングおよびフェイルオーバー設定のスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image22.png["iSCSIソフトウェアアダプタ vmhba64 が選択された ESXi のストレージアダプタタブのスクリーンショット"]

. [ 動的ターゲット ] で、 [ 動的ターゲットの追加 ] をクリックします。
+
image:express-c-series-c190-deploy_image23.png["4つのIPアドレスとポート3260が構成されたiSCSIダイナミックターゲットを示すスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image24.png["ポート3260を使用した4つのiSCSI動的ターゲットアドレスと「構成の保存」ボタンのスクリーンショット"]
...
. vSwitch0 を選択します。
+
image:express-c-series-c190-deploy_image25.png["ESXi の vSwitch0 ページのスクリーンショット。MTU 9000 とアクティブな vmnic2 および vmnic4 アダプタが表示されています。"]
...
+
image:express-c-series-c190-deploy_image26.png["ESXi のネットワークページで「ポートグループ」タブが選択され、「ポートグループの追加」が使用可能な状態のスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image27.png["ESXi ネットワーク VMkernel NIC タブに vmk0 と vmk1 インターフェイスが表示されているスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image28.png["静的 IPv4 設定と vMotion が選択された状態の「VMkernel NIC の追加」ダイアログのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image29.png["VMware ESXi のストレージページのスクリーンショット。「新しいデータストア」ボタンが表示されている状態"]

. マウント NFS データストアを選択します。
+
image:express-c-series-c190-deploy_image30.png["新規データストアウィザードで作成タイプとして「マウント NFS データストア」が選択されているスクリーンショット"]
...
. [ 保存 ] をクリックします .
+
image:express-c-series-c190-deploy_image31.png["NTPサービスが実行中で3つのNTPサーバーが設定されているESXiの時刻と日付の設定を示すスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image32.png["VMware ESXi のシステムスワップ管理ページのスクリーンショット（スワップ設定が表示されている）"]

. 設定の編集をクリックします。データストアのオプションから 'infra_swap' を選択します
+
image:express-c-series-c190-deploy_image33.png["infra_swapをデータストアとして選択した編集スワップ構成ダイアログと保存ボタンおよびキャンセルボタンのスクリーンショット"]
