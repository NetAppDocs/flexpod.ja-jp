...
+
image:express-c-series-c190-deploy_image49.png["vSphere Client の「OVF テンプレートのデプロイ」で、「開く」ダイアログに VSC OVF ファイルを選択した状態のスクリーンショット"]

. VM 名を入力し、導入先のデータセンターまたはフォルダを選択します。次へをクリックします。
+
image:express-c-series-c190-deploy_image50.png["VM名 FlexPod-VSC と FlexPod-Datacenter が選択された状態の「OVF テンプレートのデプロイ」ウィザードのスクリーンショット"]

. 「 FlexPod - Cluster ESXi 」クラスタを選択し、「 Next 」をクリックします。
. 詳細を確認し、 [ 次へ ] をクリックします。
+
image:express-c-series-c190-deploy_image51.png["NetApp VSC 仮想アプライアンス OVF テンプレートを展開するための詳細の確認手順を示すスクリーンショット"]

. [Accept （同意） ] をクリックしてライセンスを受け入れ、 [Next] をクリックします。
. シンプロビジョニング仮想ディスク形式と NFS データストアの 1 つを選択します。次へをクリックします。
+
image:express-c-series-c190-deploy_image52.png["シンプロビジョニングと NFS データストアを含むストレージの選択ページが表示された OVF テンプレートのデプロイのスクリーンショット"]

. [Select Networks] ( ネットワークの選択 ) から宛先ネットワークを選択し、 [Next] ( 次へ ) をクリックします。
+
image:express-c-series-c190-deploy_image53.png["OVF テンプレートのデプロイの「ネットワークの選択」ステップで NAT を MGMT-Network にマッピングしているスクリーンショット"]

. テンプレートのカスタマイズで、 VSC 管理者パスワード、 vCenter 名または IP アドレス、およびその他の設定の詳細を入力し、次へをクリックします。
+
image:express-c-series-c190-deploy_image54.png["vCenter 登録フィールドとネットワークプロパティを含む「OVF テンプレートのデプロイ」テンプレートのカスタマイズページのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image55.png["VMware Toolsをインストールしてから VSC、VASA Provider、および SRA のインストールを続行するよう指示を表示している VM コンソール"]
...
+
image:express-c-series-c190-deploy_image56.png["vSphere Client メニューのスクリーンショット（Virtual Storage Console が強調表示されています）"]
...
+
image:express-c-series-c190-deploy_image57.png["アップロード用に NetAppNasPlugin.vib ファイルが選択された VSC NFS VAAI Tools のスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image58.png["NetApp VSC メニューで「VMware VAAI 用 NFS プラグインのインストール」が選択されている状態を示すスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image59.png["NetApp VSCの「Set Recommended Values」が選択されたvSphere Clientのホストの「Actions」メニューのスクリーンショット"]

. 選択した vSphere ホストに適用する設定を確認してください。[OK] をクリックして設定を適用します。
+
image:express-c-series-c190-deploy_image60.png["選択されたHBA/CNAアダプタ、MPIO、およびNFS設定を含む「推奨値の設定」ダイアログ"]

+
image:express-c-series-c190-deploy_image61.png["ストレージシステムの検出が成功すると、ESXi ホスト設定が変更されたことを示す成功ダイアログが表示されます。"]
