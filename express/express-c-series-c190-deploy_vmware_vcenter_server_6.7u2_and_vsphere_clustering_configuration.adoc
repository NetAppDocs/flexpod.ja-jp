...
.  `+https://<<FQDN or IP of vCenter>>/vsphere-client/+`に移動します。
...
. [OK] をクリックします。
+
image:express-c-series-c190-deploy_image45.png["DRS と vSphere HA が有効で vSAN が無効になっている「新しいクラスタ」ダイアログのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image46.png["vSphere Client のホスト追加ウィザードのスクリーンショット（ホスト FQDN 入力フィールドと「次へ」ボタンを含む）"]
...
.  `https://<vCenter IP>:5480`にログインし、ユーザー名に「root」と入力して、rootパスワードを入力してください。
. services をクリックして、 VMware vSphere ESXi Dump Collector を選択します。
. VMware vSphere ESXi Dump コレクタサービスを開始します。
+
image:express-c-series-c190-deploy_image47.png["VMware vSphere ESXi Dump Collector が選択され、「開始」が利用可能な状態のアプライアンス管理サービスのスクリーンショット"]
...
+
image:express-c-series-c190-deploy_image48.png["ESXiのコアダンプ構成コマンドとnetdumpサーバーの検証を表示するターミナル"]
