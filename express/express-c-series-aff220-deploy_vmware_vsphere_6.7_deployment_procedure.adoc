...
+
image:express-c-series-aff220-deploy_image21.png["Cisco CIMC サーバーの BIOS ページのスクリーンショット（上部メニューに「KVM を起動」リンクあり）"]
...
=== ESXi 6.7 Cisco カスタム イメージのダウンロード

. カスタム ISO の https://support.broadcom.com/group/ecx/downloads["Broadcomサポートポータルのダウンロードサイト"^]に移動します。
...
+
image:express-c-series-aff220-deploy_image22.png["容量15.00 GiBのNETAPP LUN C-Modeストレージデバイスを選択しているESXiインストーラーのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image23.png["vSwitch0用にvmnic0とvmnic1が選択されたESXi ネットワークアダプタメニューのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image24.png["ESXi ネットワーク仮想スイッチタブに vSwitch0 と iScsiBootvSwitch が表示されているスクリーンショット"]
...
** Create をクリックします。 .
+
image:express-c-series-aff220-deploy_image25.png["静的IPv4設定とMTU 9000でiScsiBootPG-Bを構成する「VMkernel NICの追加」フォームのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image26.png["iSCSIソフトウェアアダプタvmhba64が選択されたVMware ESXiストレージアダプタページのスクリーンショット"]

. [ 動的ターゲット ] で、 [ 動的ターゲットの追加 ] をクリックします。
+
image:express-c-series-aff220-deploy_image27.png["「動的ターゲット」の下に「動的ターゲットの追加」が表示された iSCSI の設定ダイアログのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image28.png["4つのiSCSI動的ターゲットIPアドレスがそれぞれポート3260を使用していることを示すスクリーンショット"]
...
. vSwitch0 を選択します。
+
image:express-c-series-aff220-deploy_image29.png["VMware ESXi の vSwitch0 ページのスクリーンショット。MTU 1500 と 2 つのアクティブな物理アダプタが表示されています。"]
...
+
image:express-c-series-aff220-deploy_image30.png["ESXi のネットワークページで「ポートグループ」タブが選択され、「ポートグループの追加」が使用可能な状態のスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image31.png["ESXi ネットワーク VMkernel NIC タブに vmk0 と vmk1 アダプタが表示されているスクリーンショット"]
...
** Create をクリックします。 .
+
image:express-c-series-aff220-deploy_image32.png["NFS ネットワーク ポート グループを静的 IPv4 設定で構成する VMkernel NIC の追加フォームのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image33.png["静的 IPv4 設定と vMotion が選択された状態の「VMkernel NIC の追加」ダイアログのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image34.png["VMware ESXi のストレージページのスクリーンショット。「新しいデータストア」ボタンが表示されている状態"]

. マウント NFS データストアを選択します。
+
image:express-c-series-aff220-deploy_image35.png["新規データストアウィザードで作成タイプとして「マウント NFS データストア」が選択されているスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image36.png["ESXi の「システム時刻と日付の管理」ページのスクリーンショット（NTP クライアントのステータスと NTP サーバーが表示されています）"]
...
. [ 保存 ] をクリックします .
+
image:express-c-series-aff220-deploy_image37.png["NTPが有効になっていてNTPサーバーが入力された時刻設定編集ダイアログのスクリーンショット"]
...
+
image:express-c-series-aff220-deploy_image38.png["VMware ESXi のシステムスワップ管理ページのスクリーンショット（スワップ設定が表示されている）"]

. 設定の編集をクリックします。データストアのオプションから infra_swap を選択します。
+
image:express-c-series-aff220-deploy_image39.png["infra_swapをデータストアとして選択した編集スワップ構成ダイアログと保存ボタンおよびキャンセルボタンのスクリーンショット"]
...
. ESXi ホストをリブートします。
+
image:express-c-series-aff220-deploy_image40.png["ターミナルに NetApp NFS プラグイン VIB のインストール成功と再起動が必要なメッセージが表示されている"]
