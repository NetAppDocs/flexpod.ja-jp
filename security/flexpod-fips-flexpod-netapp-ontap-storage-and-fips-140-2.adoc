...
image:flexpod-fips-image3.png["暗号化された書き込みデータと復号化された読み取りデータが、ネットワーク、WAFL、RAID、および暗号化モジュールを備えたストレージを経由して流れる様子を示す図"]
...
image:flexpod-fips-image4.png["ネットワーク、WAFL、RAID、およびストレージ暗号化モジュール層を介した暗号化と復号化を示す図"]
...
image:flexpod-fips-image5.png["キーマネージャ、cryptomod、およびcryptomod_fipsコンポーネントを含むONTAP暗号境界を示す図"]
