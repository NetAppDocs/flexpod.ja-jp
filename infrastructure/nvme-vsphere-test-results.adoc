...
image:nvme-vsphere-image3.png["FlexPod AFF A800 8KB ランダム読み取り IOPS とレイテンシの関係を示すグラフ（0.76 ms で 1.25M IOPS を達成）"]
...
image:nvme-vsphere-image4.png["FlexPod AFF A800 の 8KB ランダム書き込みレイテンシが IOPS とともに増加することを示すグラフ"]
...
image:nvme-vsphere-image5.png["FlexPod AFF A800 8KB 80:20レイテンシが1M IOPSで0.95 msに上昇するチャート"]

テストでは、 100 万 IOPS を超えるシステムを達成し、サーバ側のレイテンシを 1 ミリ秒未満に抑えていることがわかりました。

ブロックサイズ 64KB 、シーケンシャルリード 100% の場合、以下の結果が得られました。

image:nvme-vsphere-image6.png["FlexPod AFF A800 の 64KB シーケンシャル読み取りレイテンシが IOPS の増加に伴い 0.2ms から 0.78ms に上昇することを示すチャート"]

テストでは、サーバ側のレイテンシを 1 ミリ秒未満に抑えながら約 25 万 IOPS を達成したことがわかりました。

ブロックサイズ 64KB とシーケンシャルライト 100% の場合、結果は次のようになりました。

image:nvme-vsphere-image7.png["FlexPod AFF A800 64KB シーケンシャル書き込みレイテンシが IOPS の増加に伴って上昇することを示すチャート"]
