...
+
image:cloud-fabricpool_image14.png["OnCommand System Manager のナビゲーションメニューで「アプリケーションと階層」が選択されているスクリーンショット"]

. [ ストレージ階層 ] をクリックします。
. アグリゲートをクリックします。
. アクションをクリックし、クラウド階層の接続を選択します。
+
image:cloud-fabricpool_image15.png["「クラウドティアを添付」が選択された状態の「その他の操作」メニューを示すスクリーンショット"]
