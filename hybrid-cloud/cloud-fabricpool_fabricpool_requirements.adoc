...
[role="lead"]
FabricPoolには ONTAP 9.2 以降が必要です。追加の FabricPool 要件は、使用されている ONTAP のバージョンと接続されているクラウド階層によって異なります。ONTAP 9.8 より前のリリースでは、FabricPool は SSD ローカル階層でのみサポートされています。HDD アグリゲートのサポートは ONTAP 9.8 で追加されました。NetApp AFF C190 など、固定された比較的小容量のエントリーレベル AFF プラットフォームの場合、FabricPool は非アクティブなデータをクラウド階層に移動する際に非常に効果的です。
...
* NetApp AFF
* NetApp FAS FabricPoolは、ONTAP 9.2を実行可能なすべてのプラットフォームでサポートされていますが、以下の場合は除きます：
+
** FAS8020
...
NOTE:  https://hwu.netapp.com/DetailViews/Home.aspx["NetApp Hardware Universe の略"^]で、FabricPoolでサポートされているプラットフォームの最新情報を参照してください。

* NetApp ONTAP Select
+

NOTE: NetAppでは、オールSSDのFabricPoolローカル階層を使用することを推奨しています。
...
** NetApp StorageGRID 10.3以降
+

NOTE: Glacier Flexible RetrievalおよびGlacier Deep Archiveはサポートされません。
...
** Azure NetApp Files
** Google Cloud 用 NetApp Cloud Volumes
...
image:cloud-fabricpool_image6.png["2つのNetApp AFF A300コントローラーがクラスター間LIFをAWS S3に接続していることを示す図"]
...
image:cloud-fabricpool_image7.png["ONTAP CLI オブジェクトストアプロファイラの結果（PUT および GET のレイテンシとスループットを示す）のスクリーンショット"]
...
FabricPoolライセンス（既存のライセンスの追加容量を含む）は、1TB単位で購入できます。
