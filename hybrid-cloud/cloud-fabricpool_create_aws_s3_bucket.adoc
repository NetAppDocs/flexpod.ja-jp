...
image:cloud-fabricpool_image10.png["有効および無効な FabricPool アグリゲートとバケットのアタッチメント構成を示す図"]
...
+
image:cloud-fabricpool_image11.png["AWS コンソールの検索画面のスクリーンショット。クラウド内の S3 スケーラブルストレージの結果がハイライト表示されている。"]

. S3 ホームページで、バケットの作成を選択します。
. DNS 準拠の名前を入力し、バケットを作成するリージョンを選択します。
+
image:cloud-fabricpool_image12.png["AWSのバケット作成ダイアログのスクリーンショット（バケット名、リージョン、作成ボタンを含む）"]
