...
このハイブリッドクラウドソリューションは、サポートされているバージョンのソフトウェア、ファームウェア、ハードウェアを実行しているFlexPod環境に拡張できます。link:http://support.netapp.com/matrix/["NetApp Interoperability Matrix Tool で確認できます"^] 、link:https://ucshcltool.cloudapps.cisco.com/public/["UCSハードウェアおよびソフトウェアの互換性"^] 、 そしてlink:https://www.vmware.com/resources/compatibility/search.php["VMware Compatibility Guide 』を参照してください"^]。
