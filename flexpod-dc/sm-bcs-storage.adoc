...
image:sm-bcs-image22.png["2つのサイトでNexus AおよびNexus BスイッチにAFF A250が接続されていることを示す図"]
...
image:sm-bcs-image23.png["Nexusに接続された AFF A250 コントローラポート、インターフェースグループ、VLANポート、および LIF を示す図"]
...
image:sm-bcs-image24.png["B200 M5サーバーからAFF A250コントローラーへの冗長iSCSI SANブート接続を示す図"]
...
image:sm-bcs-image25.png["2つのIPアドレスフィールドを含むONTAP System Managerの「クラスタ間インターフェースの追加」ダイアログのスクリーンショット"]

2 つのクラスタ間にピア関係を設定するには、次の手順を実行します。

. 1 つ目のクラスタでクラスタピアパスフレーズを生成
+
image:sm-bcs-image26.png["クラスター ピアの権限、有効性、およびコピーされたパスフレーズを含むパスフレーズ生成ダイアログのスクリーンショット"]

. 2 番目のクラスタで Peer Cluster オプションを呼び出し、パスフレーズとクラスタ間 LIF の情報を指定します。
+
image:sm-bcs-image27.png["ONTAP System Manager のピア クラスタダイアログにパスフレーズとクラスタ間 LIF アドレスが表示されているスクリーンショット"]

. System Manager Protection > Overview ペインには、クラスタピアの情報が表示されます。
+
image:sm-bcs-image28.png["クラスタ ピアとメディエーターのステータスを示す ONTAP System Manager の保護概要のスクリーンショット"]
...
image:sm-bcs-image29.png["ONTAP System Manager のメディエーター接続フィールドを含むメディエーター設定ダイアログのスクリーンショット"]

必要な情報を入力すると、設定された ONTAP メディエーターが System Manager の Protection > Overview ペインに表示されます。

image:sm-bcs-image30.png["構成済みのメディエーターのステータスが表示されているONTAP System Manager の保護概要ペインのスクリーンショット"]
...
image:sm-bcs-image31.png["ソースボリューム esxi_a とデスティネーション クラスタ aff-a250-b を使用した Protect Consistency Group ダイアログのスクリーンショット"]
...
image:sm-bcs-image32.png["サイト A における 2 つの正常な同期済み整合性グループを示す ONTAP System Manager 関係のスクリーンショット"]

このスクリーンショットは、サイト B における整合グループ関係を示しています

image:sm-bcs-image33.png["サイト B における ONTAP System Manager の関係を示すスクリーンショット。正常で同期されている 2 つの整合性グループが表示されています。"]

次のスクリーンショットは、 cg_infra_datastore_b グループの整合グループ関係の詳細を示しています。

image:sm-bcs-image34.png["cg_infra_datastore_bの整合グループ関係の詳細を表示するONTAP System Managerのスクリーンショット"]
...
image:sm-bcs-image35.png["Infra-SVM-a の ONTAP ボリューム情報のスクリーンショット（RW、DP、LS ボリュームを含む）"]

このスクリーンショットは、サイト B のボリューム情報を示しています

image:sm-bcs-image36.png["Infra-SVM-b の ONTAP ボリューム表示出力のスクリーンショット（デスティネーション DP ボリュームを含む 9 つのボリュームが表示されています）"]
...
image:sm-bcs-image37.png["サイト A の iSCSI igroup と LUN マッピングを示す ONTAP CLI 出力のスクリーンショット"]

このスクリーンショットは、サイト B のクラスタのホスト igroup と LUN マッピングを示しています。

image:sm-bcs-image38.png["サイト B の igroup および iSCSI VMware ホストの LUN マッピング コマンド出力のスクリーンショット"]
