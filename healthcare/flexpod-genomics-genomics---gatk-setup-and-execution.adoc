...
image:flexpod-genomics-image8.png["サンプル採取からシーケンス解析、バリアント解析、電子カルテ統合、予測モデルに至るゲノム解析ワークフローを示す図"]
...
+
image:flexpod-genomics-image10.png["VLAN IPアドレスの予約範囲、サブネットマスク、管理、VM、NFS、iSCSIの用途を示す表"]

. ONTAP SVM で iSCSI ベースのブート LUN を設定
+
image:flexpod-genomics-image9.png["Healthcare_SVM ストレージ VM 内の 6 つの ESXi ブート LUN を示す ONTAP System Manager のスクリーンショット"]

. LUN を iSCSI イニシエータグループにマッピングします。
+
image:flexpod-genomics-image11.png["ESXi_Boot_Lun_1がGenomicsESXi_1 iSCSIイニシエータグループにマッピングされていることを示すスクリーンショット"]

+
image:flexpod-genomics-image12.png["GenomicsESXi_2イニシエータグループにマッピングされたESXiブートLUNの詳細を示すONTAP System Managerのスクリーンショット"]

. iSCSI ブートを使用して vSphere 7.0 をインストールします。
. ESXi ホストを vCenter に登録します。
+
image:flexpod-genomics-image13.png["vSphere Client インベントリのスクリーンショット。ESXi ホストと仮想マシンを含む Genomics フォルダが表示されています。"]

. ONTAP ストレージ上で NFS データストア「 infra_datastore_nfs 」をプロビジョニングします。
+
image:flexpod-genomics-image14.png["ONTAP System Manager のスクリーンショット。選択された Healthcare_Workload_NFS_Vol NFS ボリュームがオンラインであることが表示されています。"]

. vCenter にデータストアを追加します。
+
image:flexpod-genomics-image16.png["vSphere Client で infra_datastore_nfs NFS 3 データストアの概要を表示しているスクリーンショット"]

. vCenter を使用して、 ESXi ホストに NFS データストアを追加します。
+
image:flexpod-genomics-image15.png["infra_datastore_nfs ホストが正常な状態で接続されていることを示す vSphere Client のスクリーンショット"]
...
+
image:flexpod-genomics-image17.png["/dev/sdbが/mnt/genomicsにマウントされ、使用率が2%であることを示すLinuxターミナルのスクリーンショット"]
