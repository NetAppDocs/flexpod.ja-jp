...
+
image:flexpod-genomics-image18.png["Cisco UCS Manager インベントリの GPU タブに Compute モードの P6 グラフィックカードが表示されているスクリーンショット"]

+
image:flexpod-genomics-image19.png["Compute モードの UCSB-GPU-P6-R を含む GPU インベントリを表示している Cisco UCS Manager のスクリーンショット"]
...
. このコマンドは、ESXi に https://docs.nvidia.com/vgpu/latest/grid-software-quick-start-guide/installing-configuring-vgpu-manager-guest-driver.html#installing-the-nvidia-virtual-gpu-manager-on-vmware-vsphere["NVIDIA vGPU Manager の略"^]をインストールします。
+
....
...
+
image:flexpod-genomics-image20.png["vSphere Client で Shared Direct 用に構成された NVIDIA Tesla P6 グラフィックスデバイスを示すスクリーンショット"]

+
image:flexpod-genomics-image21.png["「共有ダイレクト」が選択され、「X.Orgサーバーを再起動」にチェックが入った状態の「グラフィックデバイス設定の編集」のスクリーンショット"]
...
+
image:flexpod-genomics-image22.png["EFIファームウェアとセキュアブートが無効になっているvCenter VMブートオプションのスクリーンショット"]
...
+
image:flexpod-genomics-image23.png["vCenter の「設定の編集」詳細セクションと「構成の編集」リンクを示すスクリーンショット"]

+
image:flexpod-genomics-image24.png["pciPassthru.64bitMMIOSizeGB および pciPassthru.use64bitMMIO の値を一覧表示する「Configuration Parameters」ダイアログのスクリーンショット"]
...
+
image:flexpod-genomics-image25.png["vCenter の「Edit Settings」のスクリーンショット。PCIデバイスに対して NVIDIA GRID vGPU プロファイル grid_p6-16c が選択されている状態を示しています。"]
...
+
image:flexpod-genomics-image26.png["NVIDIA License Server のライセンスクライアントページに仮想イーサネットクライアントが1つ表示されているスクリーンショット"]
