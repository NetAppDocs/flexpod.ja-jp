...
summary: このセクションでは、 GATK からの jar ファイル出力について説明します。 
---
...
link:flexpod-genomics-genomics---gatk-setup-and-execution.html["以前のゲノム - GATK のセットアップと実行。"]
...
link:flexpod-genomics-appendix-b.html["次の例では './GATK' スクリプトを使用して GATK を実行するための出力を示します"]
